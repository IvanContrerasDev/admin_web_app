...
export function ClientsPage() {
  ...
  return <section className="flex flex-col gap-6">
    ...
    <QueryState pending={query.isPending} error={query.isError} empty={clients.length === 0} subject="los clientes" onRetry={() => void query.refetch()}>
      <div className="hidden overflow-x-auto rounded-lg border border-foreground/15 bg-background md:block"><table className="w-full text-left"><caption className="sr-only">Clientes encontrados</caption><thead className="bg-muted text-sm text-foreground/65"><tr><th className="px-5 py-3 font-semibold">Cliente</th><th className="px-5 py-3 font-semibold">Estado</th><th className="px-5 py-3 text-right font-semibold">Acción</th></tr></thead><tbody>{clients.map((client) => <tr className="border-t border-foreground/10" key={client.id}><td className="px-5 py-4 font-semibold">{client.name}</td><td className="px-5 py-4"><StatusBadge status={client.status} /></td><td className="px-5 py-4 text-right"><Link className="inline-flex min-h-11 items-center rounded-md px-3 font-semibold text-primary hover:bg-primary/10" to={`/clientes/${client.id}`}>Ver detalle</Link></td></tr>)}</tbody></table></div>
      <ul className="flex flex-col gap-3 md:hidden" aria-label="Clientes encontrados">{clients.map((client) => <li className="rounded-lg border border-foreground/15 bg-background p-4" key={client.id}><div className="flex items-start justify-between gap-3"><p className="min-w-0 break-words font-semibold">{client.name}</p><StatusBadge status={client.status} /></div><Link className="mt-4 flex min-h-11 items-center justify-center rounded-md border border-primary px-4 font-semibold text-primary hover:bg-primary/10" to={`/clientes/${client.id}`}>Ver detalle</Link></li>)}</ul>
    </QueryState>
